    ...
    for name, value in params.items():
        if not value or str(value).strip() == "":
            raise HTTPException(
                ...
                detail="""參數不能為空，顯示必填參數。
                人員上工需要傳入(1)員工的唯一辨識碼，(2)具體的工作位置或設備編號。
                人員下工需要傳入(1)員工的唯一辨識碼，(2)具體的工作位置或設備編號。
                工單進站需要傳入(1)工單或生產任務編號，(2)接收該工單的機台或站點代碼。
                工單出站需要傳入(1)工單或生產任務編號，(2)接收該工單的機台或站點代碼。"""
            )
